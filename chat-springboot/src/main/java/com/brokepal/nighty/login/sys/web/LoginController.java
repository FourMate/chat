...
import com.brokepal.nighty.login.sys.service.LoginService;
...
public class LoginController {
    ...
    public ResponseEntity login(HttpServletRequest req, HttpServletResponse resp) throws RequestParamException {
        ...
        do {
            ...
            //判断密码是否正确
            String str_privateKey = loginService.getPrivateKey(sessionId);
            ...
            String passwordMD5 = loginService.MD5Encrypt(srcPassword,salt);
            if (!user.getPassword().equals(passwordMD5)) { //如果密码错误
                result = OperationResult.buildFailureResult("密码错误");
                loginService.addFailCount(username);
                break;
            }

            //通过所有验证，登录成功
            loginService.clearFailInfo(username);
            String token = SecurityUtil.generateToken(user.getUsername(),srcPassword); //生成token
            loginService.login(username,sessionId,token,keepPassword);
    ...
    public ResponseEntity isLogin(@RequestParam("username") String username,
        ...
        OperationResult result;
        do {
            IsLoginResult isLoginResult = loginService.isLogin(sessionId, token);
